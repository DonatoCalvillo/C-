...
Modelo::Modelo(ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, WCHAR textura[], const char* obj, vector3 rot, vector3 esc, vector3 tras)
{
	//copiamos el device y el device context a la clase terreno
	d3dContext = D3DContext;
	d3dDevice = D3DDevice;
	//Cargamos esc, rot, tras
	this->rotacion = rot;
	this->escalacion = esc;
	this->traslacion = tras;

	//for (size_t i = 0; i < 3; i++) this->escalacion[i] = esc[i];
	//for (size_t i = 0; i < 3; i++) this->traslacion[i] = tras[i];
	//for (size_t i = 0; i < 3; i++) this->rotacion[i] = rot[i];
	//aqui cargamos las texturas de alturas y el cesped
	CargaParametros(textura, obj);
}
...
bool Modelo::CompileD3DShader(WCHAR* filePath, char* entry, char* shaderModel, ID3DBlob** buffer)
{
...
bool Modelo::CargaParametros(WCHAR* diffuseTex, const char* obj)
{
	HRESULT d3dResult;
...
void Modelo::Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, vector3 rot, vector3 trans, vector3 esc, float faa)
{
	...
	D3DXMatrixTranslation(&translationMat, trans.x, trans.y, trans.z);
	D3DXMATRIX scaleMat;
	D3DXMatrixScaling(&scaleMat,esc.x, esc.y, esc.z);
	D3DXMATRIX ry;
	D3DXMatrixRotationY(&ry, 0.01);
	viewMatrix *= ry;

	D3DXMATRIX worldMat = scaleMat * rotationMat * translationMat ;//= rotationMat * translationMat;
...
void Modelo::CargaModelos(const char* obj)
{
