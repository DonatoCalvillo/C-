...
public:
	...
	D3DXMATRIX UpdateCam(float vel, float arriaba, float izqder)
	{
		...
		if (firstperson) {
			...
			D3DXVECTOR3 tempRefFront = refFront * 10;

			D3DXVECTOR3 tempCam = posCam;
			D3DXVECTOR3 temp = hdveo;
			temp = tempCam + tempRefFront;
			D3DXMatrixLookAtLH(&vista, &temp, &tempCam, &refUp);
		}
		D3DXMatrixTranspose( &vista, &vista );
		return vista;
	}
