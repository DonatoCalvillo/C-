...
private:
	...
	ID3D11ShaderResourceView* colorMap;		//dirt
	ID3D11ShaderResourceView* colorMap2;	//dirt norm
	ID3D11ShaderResourceView* colorMap3;	//rock
	ID3D11ShaderResourceView* colorMap4;	//rock norm
	ID3D11ShaderResourceView* colorMap5;	//snow
	ID3D11ShaderResourceView* colorMap6;	//snow norm
	ID3D11ShaderResourceView* rgbMap;		//blend
	ID3D11ShaderResourceView* blendMap;		//heighmap
	ID3D11SamplerState* colorMapSampler;

	ID3D11Buffer* viewCB;
	ID3D11Buffer* projCB;
	ID3D11Buffer* worldCB;
	ID3D11Buffer* FAA; //FAA
...
public:
	TerrenoRR(int ancho, int alto, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext,WCHAR* altura, WCHAR* textura, WCHAR* texturaN, WCHAR* textura2, WCHAR* textura2N, WCHAR* textura3, WCHAR* textura3N, WCHAR* RGB)
	{
		...
		CargaParametros(altura, textura, texturaN, textura2, textura2N, textura3, textura3N, RGB, 30.0f); //escala de la textura y textura del terreno
	...
	bool CargaParametros(WCHAR* heightTex,WCHAR* diffuseTex, WCHAR* texturaN, WCHAR* textura2, WCHAR* textura2N, WCHAR* textura3, WCHAR* textura3N, WCHAR* RGB, float tile)
	{
		HRESULT d3dResult;
		...
		estableceIndices();
		//crea los accesos de las texturas para los shaders ped
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, texturaN, 0, 0, &colorMap2, 0);     
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, diffuseTex, 0, 0, &colorMap, 0);
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, textura2N, 0, 0, &colorMap5, 0);    
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, textura3N, 0, 0, &colorMap6, 0);    
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, heightTex, 0, 0, &blendMap, 0);			//heighmap
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, textura2, 0, 0, &colorMap3, 0);			
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, textura3, 0, 0, &colorMap4, 0);			
		d3dResult = D3DX11CreateShaderResourceViewFromFile(d3dDevice, RGB, 0, 0, &rgbMap, 0);			
	...
	void Draw(D3DXMATRIX vista, D3DXMATRIX proyeccion, float faa)
	{
		...
		d3dContext->PSSetShaderResources(2, 1, &colorMap5);
		d3dContext->PSSetShaderResources(3, 1, &colorMap6);
		d3dContext->PSSetShaderResources(4, 1, &blendMap);
		d3dContext->PSSetShaderResources(5, 1, &colorMap3);
		d3dContext->PSSetShaderResources(6, 1, &colorMap4);
		d3dContext->PSSetShaderResources(7, 1, &rgbMap);
		...
		d3dContext->VSSetConstantBuffers(3, 1, &FAA);
