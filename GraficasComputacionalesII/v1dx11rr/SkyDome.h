...
public:
	...
	bool LoadContent(WCHAR* diffuseTex)
	{
		...
		for (int x = 0; x < slices; x++)
		{
			for (int y = 0; y < stacks; y++)
			{
				...
				vertices[indiceArreglo].UV = D3DXVECTOR2(u / 3, v / 3);
			}
		}
		...
		d3dResult = (*d3dDevice)->CreateBuffer(&constDesc2, 0, &Timer);
	...
	void Render(D3DXVECTOR3 trans, D3DXVECTOR4 timer)
	{
		if (d3dContext == 0)
			return;
		...
		/*matrices->valores.x += .01;
		if (matrices->valores.x > 1) {
			matrices->valores.x = 0;
		}*/


		matrices->valores.x += 0.005;
		if (matrices->valores.x >= 1)
		{
			matrices->valores.x -= 1;
		}
		...
		(*d3dContext)->UpdateSubresource(Timer, 0, 0, &timer, 0, 0);
		//(*d3dContext)->UpdateSubresource(paddingBufferCB, 0, 0, &padding, 0, 0);
		(*d3dContext)->VSSetConstantBuffers(0, 1, &matrixBufferCB);
		(*d3dContext)->VSSetConstantBuffers(1, 1, &Timer);
		/*(*d3dContext)->VSSetConstantBuffers(1, 1, &paddingBufferCB);
		(*d3dContext)->PSSetConstantBuffers(1, 1, &paddingBufferCB);*/
